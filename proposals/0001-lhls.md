...
HTTP low-latency streaming (as opposed to alternative protocols like WebRTC) allows developers to leverage existing HTTP video delivery infrastructure, most notably edge caching. It is also more straightforward to implement in Hls.js itself. But WebRTC, while more complex, allows clients to directly connect with servers, which further reduces latency.
