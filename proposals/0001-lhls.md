...
#### Complete Segment Advertising

Q: What's the minimum amount of segments
...
(Q: Does this need to be a prefetch tag - i.e. does it break clients which don't support LHLS?)

A prefetch segment may be advertised with an `EXT-X-PROGRAM-DATE-TIME` tag. The value should be as close to capture time as possible. 
...
(Q: Can partial segments be decrypted?)

Prefetch segments may be advertised with an `EXT-X-KEY` tag. 
...
### Transforming to Complete Segments
...
- The server must advertise the transformed segment with the same URI used in the prefetch advertisement. (Q: Is URI allowed to change in the spec?)
- The server must append the `EXTINF` metadata tag to the transformed segment.
- If the prefetch segment was advertised with an `EXT-X-PREFETCH-DISCONINUITY` tag, the transformed segment must be advertised with an `EXT-X-DISCONTINUITY` tag.
- If the prefetch segment was advertised with an `EXT-X-PROGRAM-DATE-TIME` tag, the transformed segment must be advertised with an identical `EXT-X-PROGRAM-DATE-TIME` tag.
