...
The current latency between capture and playback ("glass to glass") is prohibitive for live interactivity, and provides a poor experience for time-sensitive live events (such as sports). The introduction of low-latency streaming via HTTP in Hls.js would allow developers to build a cost-effective solution on top of a free, open, and proven client. Furthermore, the introduction of a progressive streaming pipeline in Hls.js would improve performance not only for live but for video-on-demand (VOD). Progressively buffering segments allows Hls.js to reduce its TTFF (time to first frame), and by the same token reduce the rebuffering time caused by network instability.

Our goal is to reduce latency to about two seconds. The objective of this proposal is to gain consensus on our implementation before we build it. We want consensus from developers interested in using Hls.js as their LHLS client - both backend and frontend developers. Consensus is sought because Hls.js cannot afford to build a bespoke implementation of LHLS for each service which offers it. That approach does not scale, nor is it appropriate for an open-source project to meet the needs of an individual. Through the peer review of this proposal we hope to arrive at a technically sound solution which addresses the varied requirements of our developer community - to build something that developers can and want to use.
...
## How to advertise Prefetch Segments?
